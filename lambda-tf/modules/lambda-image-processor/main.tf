...
# This module creates a Lambda function specifically for processing images from S3
...
data "archive_file" "lambda_zip" {
  ...
  output_path = "${path.module}/lambda-image-processor.zip"
...
# Custom policy for S3, image processing permissions
    ...
    widgets = [
      {
        ...
        properties = {
          ...
          title  = "Image Processor Lambda Metrics - ${var.function_name}"
        }
      },
      {
        type   = "metric"
        ...
        properties = {
          ...
          stat   = "Average"
          region = data.aws_region.current.name
          title  = "Image Processor Lambda Duration - ${var.function_name}"
        }
      }
    ]
  })
