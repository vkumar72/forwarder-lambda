# S3 Event Forwarder Lambda Module
# This module creates a Lambda function specifically for forwarding S3 events to SQS/SNS

# Data source for current region
data "aws_region" "current" {}

# Data source for source bucket
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = aws_lambda_function.s3_forwarder.arn
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_s3_bucket_notification" "lambda_notification" {
  ...
  lambda_configuration {
    lambda_function_arn = aws_lambda_function.s3_forwarder.arn
...
resource "aws_cloudwatch_metric_alarm" "lambda_errors" {
  ...
  dimensions = {
    FunctionName = aws_lambda_function.s3_forwarder.function_name
  }
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            ["AWS/Lambda", "Invocations", "FunctionName", aws_lambda_function.s3_forwarder.function_name],
            [".", "Errors", ".", "."],
            [".", "Throttles", ".", "."]
          ]
          period = 300
      ...
      },
      {
        ...
        properties = {
          metrics = [
            ["AWS/Lambda", "Duration", "FunctionName", aws_lambda_function.s3_forwarder.function_name]
          ]
          period = 300
          stat   = "Average"
          region = data.aws_region.current.name
